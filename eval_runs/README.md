...
| 2026-05-20T (run e)  | `c6b9255` | **91.94** | 0 | **validator benign-context override (PR #74, submission-v1)** — eliminated 9 false-911s on the test-set "burnt popcorn" over-escalation trap canary (would have cost −45 raw rubric points). Dev: 20/20 true 911s preserved, +1.06 composite from LLM-noise wins on subcategory (98)/risk (84)/auto (98.8). Test-set 800-row run: 75 → 67 911-dispatches, all 9 audited trap-canary false-911s suppressed, 4/4 verified-true emergencies still dispatching. Predictions: `eval_runs/dev_negfix.json`. |
| 2026-05-20T (run f-prime) | `8ae0116` (main, byte-identical agent code to `c6b9255`) | 90.76 | 0 | **baseline-recheck on new OpenAI key (tier-1 separate org)** — same agent code as `c6b9255`/run e, re-measured. Composite drifted −1.18 from 91.94, purely from LLM noise. Establishes the noise floor for single-run dev evals: ±~1.2 composite. Any future change must beat this margin to be measurable. Predictions: `eval_runs/dev_baseline_recheck.json`. |
| 2026-05-20T (run f)  | `risk-cap-pipe-power@3de3bd1` | 91.04 | 0 | **LEVER A — STAND DOWN.** risk.py near-tie demote: when base_risk is HIGH for a subcategory whose historical distribution is split MEDIUM/HIGH (pipe_leak, power_outage) AND no extracted urgency cue is present, demote to MEDIUM. Targeted axis worked deterministically: risk_level 84→86 (+2), hitl_f1 71.4→73.1 (+1.7). But upstream LLM noise on category (−1), subcategory (−1.5), vendor (−1), auto_res (−1.15) dragged net composite to −0.90 vs 91.94 baseline. Within the ±1.2 noise floor (see run f-prime). Edge-case risk failures unchanged at 5/16 — the original target. Decision: fails the ≥+0.3 rule; insufficient eval budget for multi-run averaging to confirm sign. WIP commit preserved on `risk-cap-pipe-power` for archaeology. Predictions: `eval_runs/dev_leverA.json`. |